...
def draw_update_ui(layout, scene):
    ...
    row = box.row()  # ← 横一列に並べる
    op1 = row.operator("brt.open_url", text=_("Check for Updates"), icon="CHECKMARK")      # 更新を確認
    op1.url = "https://github.com/Riel2982/DIVA-Blender_Addons/releases"

    row.operator("brt.execute_update", text=_("Install"), icon="IMPORT")  # インストール
    row.operator("brt.open_addon_folder", text=_("Open Addon Folder"), icon="FILE_FOLDER")        # アドオンフォルダを開く
    ...
    if False:
        ...
        if getattr(wm, "brt_new_release_available", False): # 最新リリースがあるとき
    ...
    row = box.row()
    row.prop(scene, "brt_download_folder", text=_("Path to ZIP download folder "))  # テキスト入力欄（ラベルなし）/ ZIP保存先フォルダ パス
...
class BRT_OT_ExecuteUpdate(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not pattern.match(filename):
            self.report({'WARNING'}, _("Only ZIP files starting with DIVA_BoneRenameTools can be processed"))       # DIVA_BoneRenameTools で始まるZIPファイル以外は処理できません
        ...
        try:
            ...
            if not os.path.isdir(source_folder) or not os.path.isfile(source_init):
                self.report({'WARNING'}, _("Missing DIVA_BoneRenameTools folder or __init__.py inside the ZIP file"))       # ZIP内に DIVA_BoneRenameTools フォルダまたは __init__.py が見つかりません
...
def register_properties():
    ...
    # GitHubリリース確認
    bpy.types.WindowManager.brt_new_release_available = bpy.props.BoolProperty(
        name="New Release Available",
        description="True if a newer GitHub release is available",
        default=False
    )

    bpy.types.WindowManager.brt_initialized = bpy.props.BoolProperty(
...
def unregister_properties():
    ...
    del bpy.types.WindowManager.brt_new_release_available
    del bpy.types.WindowManager.brt_initialized