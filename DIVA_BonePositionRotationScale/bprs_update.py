        ...
        if not prefs.initialized:
            ...
            if False:
                def delayed_init():
                    ...
                    if hasattr(scene, "bprs_download_folder"):
...
def draw_update_ui(layout, scene):
    ...
    row = box.row()
    row.prop(scene, "bprs_download_folder", text=_("Path to ZIP download folder"))  # テキスト入力欄（ラベルなし）/ ZIP保存先フォルダパス
