...
from .fop_update import save_settings, load_download_folder, get_latest_release_data, get_release_label, download_and_finalize
...
class FOP_AddonPreferences(bpy.types.AddonPreferences):
    bl_idname = "DIVA_FBXOperationPack"  # アドオンのフォルダ名（ハイフンや半角スペース、記号は使用不可）
...
def draw_update_ui(layout, scene):
    ...
    row = box.row()  # ← 横一列に並べる
    op1 = row.operator("fop.open_url", text=_("Check for Updates"), icon="CHECKMARK")      # 更新を確認
    op1.url = "https://github.com/Riel2982/DIVA-Blender_Addons/releases"

    row.operator("fop.execute_update", text=_("Install"), icon="IMPORT")  # インストール
    row.operator("fop.open_addon_folder", text=_("Open Addon Folder"), icon="FILE_FOLDER")        # アドオンフォルダを開く

    # アドオンの最新リリースのお知らせ
    wm = bpy.context.window_manager
    if wm.fop_new_release_available:
        ...
        if display_version:
            ...
            if download_url:
                split.operator("fop.download_latest_zip", text=("Download"))
                row.separator()

    if False:
        wm = bpy.context.window_manager
        if getattr(wm, "fop_new_release_available", False): # 最新リリースがあるとき
            release_info = get_latest_release_data()
    ...
    row = box.row()
    row.prop(scene, "fop_download_folder", text=_("Path to ZIP download folder "))  # テキスト入力欄（ラベルなし）/ ZIP保存先フォルダ パス

    # 成功時だけ表示する INFOラベル
    wm = bpy.context.window_manager
    if getattr(wm, "fop_update_completed", False):
    ...
    # 非実行時・失敗時に表示する 更新ファイルリスト
        ...
        row.operator("fop.sort_candidates_name", text="", icon="SORTALPHA")     # 名前順
        row.operator("fop.sort_candidates_date", text="", icon="SORTTIME")      # 日付順
        row.operator("fop.confirm_download_folder", text="", icon="FILE_REFRESH")  # リスト更新

        box.template_list(
            "FOP_UL_UpdateCandidateList", "",
            scene, "fop_update_candidates",
            scene, "fop_selected_candidate_index",
            rows=2      # 初期行数
        )


# --- プロパティグループ＆UIリスト -------------------------------------
# DLフォルダパスの中のZIPファイル一覧
class FOP_UpdateCandidateItem(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty()
    path: bpy.props.StringProperty()
    date: bpy.props.StringProperty()

class FOP_UL_UpdateCandidateList(bpy.types.UIList):
...
# 各種オペレーター
class FOP_OT_OpenURL(bpy.types.Operator):
    """更新を確認"""
    bl_idname = "fop.open_url"
...
class FOP_OT_DownloadLatestZip(bpy.types.Operator):
    bl_idname = "fop.download_latest_zip"
    ...
    def execute(self, context):
        ...
        if not folder or not os.path.isdir(folder):
            ...
            bpy.ops.fop.confirm_download_folder('INVOKE_DEFAULT')
...
class FOP_OT_ExecuteUpdate(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    """更新ファイルをインストール"""
    bl_idname = "fop.execute_update"
    bl_label = "Install Update File"
    bl_description = _("Select a ZIP archive beginning with DIVA_FBXOperationPack to install the update")
    # bl_options = {'UNDO'}

    # ImportHelper 用のファイル選択パス（ダイアログ）
    filepath_dialog: StringProperty(
        name="Select ZIP File",
        description=_("Choose a ZIP file starting with DIVA_FBXOperationPack"),    # DIVA_FBXOperationPackで始まるZIPファイルを選択してください
        subtype='FILE_PATH',
    )
    filename_ext = ".zip"
    ...
    def invoke(self, context, event):
        index = context.scene.fop_selected_candidate_index
        candidates = context.scene.fop_update_candidates
    ...
    def execute(self, context):
        ...
        if not pattern.match(filename):
            self.report({'WARNING'}, _("Only ZIP files starting with DIVA_FBXOperationPack can be processed"))       # DIVA_FBXOperationPack で始まるZIPファイル以外は処理できません
            context.window_manager.fop_update_completed = False
            return {'CANCELLED'}

        # 一時解凍フォルダの作成
        downloads_path = os.path.join(os.path.expanduser("~"), "Downloads")
        extract_path = os.path.join(downloads_path, "_fop_temp_extract")
        ...
        try:
            ...
            source_folder = os.path.join(extract_path, "DIVA_FBXOperationPack")
            source_init = os.path.join(source_folder, "__init__.py")
            if not os.path.isdir(source_folder) or not os.path.isfile(source_init):
                self.report({'WARNING'}, _("Missing DIVA_FBXOperationPack folder or __init__.py inside the ZIP file"))       # ZIP内に DIVA_FBXOperationPack フォルダまたは __init__.py が見つかりません
            ...
            if os.path.exists(pycache_path):
                try:
                    ...
                    if DEBUG_MODE:
                        print(f"[FOP] __pycache__ deleted: {pycache_path}")  # 古いバイトコードキャッシュを削除しました
                except Exception as e:
                    if DEBUG_MODE:
                        print(f"[FOP] Failed to delete __pycache__: {e}")

            # 更新完了ポップアップ表示
            self.report({'INFO'}, _("Update completed. Please restart Blender"))        # 更新が完了しました。Blenderを再起動してください
            context.window_manager.fop_update_completed = True
...
class FOP_OT_OpenAddonFolder(bpy.types.Operator):
    """現在のアドオンフォルダを開く"""
    bl_idname = "fop.open_addon_folder"
...
class FOP_OT_ConfirmDownloadFolder(bpy.types.Operator):
    """更新ファイルリスト更新"""
    bl_idname = "fop.confirm_download_folder"
    ...
    def execute(self, context):
        import os
        scene = context.scene
        folder = scene.fop_download_folder
        ...
        for fname in sorted(files, reverse=True):
            # if fname.startswith("DIVA_FBXOperationPack") and fname.endswith(".zip"):
            # if re.match(r"^DIVA_FBXOperationPack.*\.zip$", fname, re.IGNORECASE):     # GameBanana小文字化対応 
            if re.match(r"^DIVA_FBXOperationPack.*?( |\.)?v\d+\.\d+\.\d+(?:[ _\.-][a-zA-Z0-9]+)?(?: \(\d+\))?\.zip$", fname):  # GitHub半角スペース→ピリオド変換対応 / 自動ナンバリング対応
                full_path = os.path.join(folder, fname)
                timestamp = os.path.getmtime(full_path)
                import datetime
                date = datetime.datetime.fromtimestamp(timestamp).strftime("%Y-%m-%d %H:%M")
                item = scene.fop_update_candidates.add()
                item.name = fname
                item.path = full_path
                item.date = date
...
class FOP_OT_SortCandidatesByName(bpy.types.Operator):
    """ZIPファイル名ソート(A–Z / Z–A)"""
    bl_idname = "fop.sort_candidates_name"
    bl_label = "Sort by File Name"
    bl_description = _("Sort update files by file name. Click again to toggle order")

    def execute(self, context):
        scene = context.scene
        items = [(i.name, i.path, i.date) for i in scene.fop_update_candidates]
        scene.fop_update_candidates.clear()

        reverse = scene.fop_sort_name_desc
        for name, path, date in sorted(items, key=lambda x: x[0].lower(), reverse=reverse):
            item = scene.fop_update_candidates.add()
            item.name, item.path, item.date = name, path, date

        scene.fop_sort_name_desc = not scene.fop_sort_name_desc  # トグル切替
        return {'FINISHED'}

class FOP_OT_SortCandidatesByDate(bpy.types.Operator):
    """日時順ソート(newest ↔ oldest)"""
    bl_idname = "fop.sort_candidates_date"
    ...
    def execute(self, context):
        ...
        reverse = scene.fop_sort_date_desc
        for name, path, date in sorted(items, key=lambda x: x[2], reverse=reverse):
            item = scene.fop_update_candidates.add()
            item.name, item.path, item.date = name, path, date

        scene.fop_sort_date_desc = not scene.fop_sort_date_desc  # トグル切替
        return {'FINISHED'}


def get_classes():
    return [
        FOP_AddonPreferences,
        FOP_OT_OpenURL,
        FOP_OT_DownloadLatestZip,
        FOP_OT_ExecuteUpdate,
        FOP_OT_OpenAddonFolder,
        FOP_OT_ConfirmDownloadFolder,
        FOP_UpdateCandidateItem,
        FOP_UL_UpdateCandidateList,
        FOP_OT_SortCandidatesByName,
        FOP_OT_SortCandidatesByDate,
    ]

def register_properties():
    # ダウンロードフォルダの初期値を設定ファイルから読み込む
    bpy.types.Scene.fop_download_folder = bpy.props.StringProperty(
    ...
    bpy.types.Scene.fop_update_candidates = bpy.props.CollectionProperty(
        type=FOP_UpdateCandidateItem
    )
    bpy.types.Scene.fop_selected_candidate_index = bpy.props.IntProperty(
        name="Updated File List",        # 候補リスト選択インデックス   Selected index in candidate list
        description = _("After selecting a ZIP file, click the Install button to update the currently installed addon"),
        default=-1
    )

    # 更新完了フラグ（INFOラベル表示の制御に使用）
    bpy.types.WindowManager.fop_update_completed = bpy.props.BoolProperty(
        name="Update Completed Flag",      # 更新完了フラグ
        default=False
    )
    
    # 名前順ソートトグル式
    bpy.types.Scene.fop_sort_name_desc = bpy.props.BoolProperty(default=False)
    # 日時順ソートトグル式
    bpy.types.Scene.fop_sort_date_desc = bpy.props.BoolProperty(default=True)

    # 不要ファイル削除要フラグ
    bpy.types.WindowManager.fop_obsolete_cleanup_done = bpy.props.BoolProperty(
        name="Obsolete Cleanup Done",
        default=False
    )

    # GitHubリリース確認
    bpy.types.WindowManager.fop_new_release_available = bpy.props.BoolProperty(
        name="New Release Available",
        description="True if a newer GitHub release is available",
        default=False
    )

    bpy.types.WindowManager.fop_initialized = bpy.props.BoolProperty(
        name="Whether initialized or not",
        default=False   # 初期化未実行（True=初期化済み、以降はスキップ）
    )

    # 通知ログの重複防止
    bpy.types.WindowManager.fop_last_display_version = bpy.props.StringProperty(
...
def unregister_properties():
    del bpy.types.Scene.fop_download_folder
    del bpy.types.Scene.fop_update_candidates
    del bpy.types.Scene.fop_selected_candidate_index
    del bpy.types.WindowManager.fop_update_completed
    del bpy.types.Scene.fop_sort_name_desc
    del bpy.types.Scene.fop_sort_date_desc
    del bpy.types.WindowManager.fop_obsolete_cleanup_done
    del bpy.types.WindowManager.fop_new_release_available
    del bpy.types.WindowManager.fop_initialized
    del bpy.types.WindowManager.fop_last_display_version